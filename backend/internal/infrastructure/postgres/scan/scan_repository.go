...
func (r *ScanRepo) Create(ctx context.Context, s *model.Scan) error {
	...
	_, err := r.db.Exec(ctx,
		`INSERT INTO scans
		 (id, organization_id, neighbourhood, district, city, latitude, longitude, source, status,
		...
		s.ID, s.OrganizationID, s.Neighbourhood, s.District, s.City, s.Latitude, s.Longitude, s.Source,
		s.Status,
...
func (r *ScanRepo) GetByID(ctx context.Context, id uuid.UUID) (*model.Scan, error) {
	s, err := r.scanRow(ctx,
		`SELECT id, organization_id, neighbourhood, district, city, latitude, longitude, source, status,
		        accessibility_score, issues, street_view_url, anonymized_image_url,
		        error_message, requested_by, created_at, updated_at, completed_at
...
func (r *ScanRepo) scanRowFromRow(row pgx.Row) (*model.Scan, error) {
	...
	var issuesJSON []byte
	if err := row.Scan(
		&s.ID, &s.OrganizationID, &s.Neighbourhood, &s.District, &s.City, &s.Latitude, &s.Longitude, &s.Source, &s.Status,
		&s.AccessibilityScore, &issuesJSON, &s.StreetViewURL, &s.AnonymizedImageURL,
		&s.ErrorMessage, &s.RequestedBy, &s.CreatedAt, &s.UpdatedAt, &s.CompletedAt,
	); err != nil {
		return nil, err
